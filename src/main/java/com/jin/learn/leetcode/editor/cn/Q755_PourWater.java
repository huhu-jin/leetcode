...
class Solution {
        ...
        // 当一滴水从 K 处落下时，我们首先检查它是否可以向左流动而下降。从 i = K 开始，每次向左检查是否有 height[i - 1] <= height[i]，
        // 否则水不可能继续向左流动。如果此时有 height[i - 1] < height[i]，那么 i - 1 处就是一个可能的水停止的位置。在检查完毕之后，
        // 最小的可能的水停止的位置即为答案，如果没有这样的位置，再用相同的方法向右检查。如果仍然没有这样的位置，那么水会停在 K 处。
