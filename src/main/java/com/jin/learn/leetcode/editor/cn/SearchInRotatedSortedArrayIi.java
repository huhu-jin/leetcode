...
public class SearchInRotatedSortedArrayIi{
    ...
    // 需要了解 https://leetcode.cn/problems/search-in-rotated-sorted-array/solution/sou-suo-xuan-zhuan-pai-xu-shu-zu-by-leetcode-solut/
class Solution {
    public boolean search(int[] nums, int target) {
        ...
        while (l <= r) {
            ...
            if (nums[l] == nums[mid] && nums[mid] == nums[r]) {
                l++;
                r--;
            }else  if(nums[l] <= nums[mid]){ // 判断 前后部分有序
                if (nums[l] <= target && target < nums[mid]) { // 在有序中
                    r = mid-1;
                }else {
                    l = mid +1;
                }
...
//leetcode submit region end(Prohibit modification and deletion)
    ...
    public void testCase(){
        new Solution().search(new int[]{5, 6, 7, 1, 2, 3, 4}, 5);
    }
